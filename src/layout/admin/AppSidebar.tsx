...
type NavItem = {
  ...
  path?: string;
  handle?: () => void;
  subItems?: { name: string; path?: string; handle?: () => void; pro?: boolean; new?: boolean }[];
};
...
const AppSidebar: React.FC = () => {
  ...
  const { toggleDialogLogout } = useAuth();
            ...
            nav.path ? (
              ...
              : (
                <button
                  ...
                  onClick={() => {
                    if (nav?.handle) {
                      nav?.handle(); // Call the handle function if it exists
                    }
                  }}
                  className={`menu-item group ${isActive(nav.path) ? "menu-item-active" : "menu-item-inactive"}`}
                >
                ...
                {nav.subItems.map((subItem) => (
                  <li key={subItem.name}>
                    {subItem.path ? (
                      ...
                      <button
                        onClick={() => {
                          if (subItem?.handle) subItem?.handle();
                        }}
